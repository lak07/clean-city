...
export default class Mumbai extends TrackerReact(React.Component){
constructor() {
	super();

	this.state={
		subscription: {
			comments : Meteor.subscribe("allCommenttM"),
...
getVotes(){
	cvotes = Vote.find({City: "Mumbai"}).fetch();
	if(cvotes.length<1){
		return 'lodaing...';
	}
...
subcoment(event){
	...
	Meteor.call('addcoments',coment,'Mumbai',()=>{
...
let votes = this.getVotes();
 return(
	<div className="main-layout" >
		...
		<div>
			<p> Current votes for Mumbai are { votes }	</p>		
		</div>	
			<ul>
