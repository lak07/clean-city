...
export default class Banglore extends TrackerReact(React.Component){
constructor() {
	super();

	this.state={
		subscription: {
			comments : Meteor.subscribe("allCommenttB"),
...
getVotes(){
	cvotes = Vote.find({City: "Banglore"}).fetch();
	if(cvotes.length<1){
		return 'lodaing...';
	}
...
subcoment(event){
	...
	Meteor.call('addcoments',coment,'Banglore',()=>{
...
let votes = this.getVotes();
 return(
	<div className="main-layout" >
		...
		<div>
			<p> Current votes for Banglore are { votes }	</p>	
		</div>	
			<ul>
